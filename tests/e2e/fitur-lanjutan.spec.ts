...
test('pengaturan pengingat menampilkan keadaan VAPID dengan jujur', async ({ browser }) => {
	...
	const pesan = page.getByText(/Kunci VAPID belum dipasang|Peramban ini tidak mendukung/);
	const tombol = page.getByRole('button', { name: 'Nyalakan pengingat' });
	await expect(pesan.or(tombol).first()).toBeVisible({ timeout: 30_000 });
});

test('halaman passkey tampil dan melaporkan dukungan peramban', async ({ browser }) => {
	const page = await halamanBaru(browser);
	await daftar(page);
	...
	await expect(
		page.getByText(/Belum ada passkey terdaftar|Peramban ini tidak mendukung passkey/)
