...
|---|---|---|
| `SERVER_AUTH_PEPPER` | Pepper untuk `users.auth_hash` | **Semua akun tidak bisa masuk lagi.** Cadangkan terpisah dari basis data |
...
Cadangkan `SERVER_AUTH_PEPPER` di tempat yang **berbeda** dari basis data. Cadangan yang menyimpan
...
dihitung ulang tanpa `authKey` mentah, dan peladen tidak pernah menyimpannya.

Prosedur:

1. Tambahkan `SERVER_AUTH_PEPPER_LAMA` berisi nilai lama, dan isi `SERVER_AUTH_PEPPER` dengan nilai
   baru.
2. Deploy versi peladen yang memverifikasi login dengan pepper baru **lalu** pepper lama.
...
### 5.1 Dugaan kebocoran basis data
...
3. Kirim pemberitahuan ke semua pengguna. Sampaikan apa yang bocor dan apa yang tidak, dengan tabel
   "Apa yang diketahui peladen" dari `THREAT-MODEL.md` disalin apa adanya.
...
Moderasi **tidak** berlaku pada catatan privat, dan itu memang disengaja. Peladen tidak bisa dan
tidak akan memindainya.

## 6. Observability

Cloister tidak memakai layanan pelacakan galat pihak ketiga. Alasannya ada di
[`ADR/0004-observability-tanpa-pihak-ketiga.md`](ADR/0004-observability-tanpa-pihak-ketiga.md).

Yang tersedia:

- Log galat lokal di IndexedDB, maksimal 200 entri rolling, tanpa isi catatan
- Pengiriman laporan galat manual dengan pratinjau persis apa yang akan dikirim
- Metrik agregat di peladen: kode galat per endpoint, latensi p50 dan p95, tanpa ID pengguna
...
Tier gratis: 20.000 catatan dan 2 GB lampiran per akun, ditegakkan di peladen.
...
- Ciphertext dibatasi 128 KB per catatan lewat CHECK constraint di basis data
