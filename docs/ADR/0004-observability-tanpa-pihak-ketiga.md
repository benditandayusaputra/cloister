# ADR 0004 — Observability tanpa layanan pelacakan error pihak ketiga
...
adalah SDK pelacakan error (Sentry dan sejenisnya) yang mengirim stack trace, breadcrumb, state
...
- Pesan error sering memuat potongan data yang menyebabkannya.

Memasang SDK semacam itu berarti membangun arsitektur zero-knowledge lalu meletakkan pipa langsung
dari plaintext ke server pihak ketiga. Konfigurasi ketat bisa mengurangi risikonya, tapi ia menjadi
sesuatu yang harus terus dijaga benar, dan satu regresi diam-diam sudah cukup untuk membatalkan
seluruh janji produk.

## Keputusan

Tidak memakai layanan pelacakan error pihak ketiga sama sekali. Sebagai gantinya:

1. **Log error lokal.** Error dicatat ke IndexedDB, maksimal 200 entri rolling, berisi stack trace,
   versi build, dan nama rute. Isi catatan, judul, dan tag tidak pernah ikut.
2. **Pengiriman manual dengan preview.** Tombol "Kirim laporan error" menampilkan **persis** apa
   yang akan dikirim, dalam bentuk teks yang bisa dibaca dan disunting pengguna sebelum menekan
   kirim.
3. **Metrik agregat tanpa identitas.** Server mencatat kode error per endpoint dan latensi p50 dan
...
- Kompensasinya digeser ke sisi yang tidak melanggar batas: cakupan tes yang lebih tinggi, vektor
  uji kriptografi tetap, tes E2E lintas browser, dan pesan error yang cukup jelas supaya pengguna
