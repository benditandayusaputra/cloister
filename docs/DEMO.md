...
Kredensial akun demo ada di file `AKUN-CONTOH.txt` yang **tidak ikut masuk repositori** (ia ada di
`.gitignore` dengan sengaja — sandi yang dikomit adalah sandi yang bocor). File itu dilampirkan
...
Untuk mengisi feed publik `/baca` dengan penulis dan catatan contoh:

```bash
pnpm seed:publik
```

Skrip ini membuat dua akun penulis sungguhan lewat browser (@arunika dan @kelana, kredensialnya
ditambahkan ke `AKUN-CONTOH.txt`), lalu menyisipkan sembilan catatan publik lewat SQL — dan itu
sesuai desain: tabel `public_entries` memang plaintext karena isinya sudah sengaja dikeluarkan
dari enkripsi oleh penulisnya. Akun utama ikut diberi nama pena `@bendi` dan dua catatan terbit.
...
| `/tentang` | Cerita nama, penjelas arsitektur, tabel "yang kami simpan dan yang tidak" |
...
`/bukti` butuh login karena ia menampilkan baris database milik pengguna sendiri.

## 3. Skenario 10 menit

| Waktu | Aksi | Yang harus dipahami juri |
|---|---|---|
| 0:00–0:45 | Masalah, satu kalimat inti produk, penjelasan nama | Kenyamanan awan tidak seharusnya mewajibkan isi privat terbaca berada di server |
...
| 3:30–5:00 | **Matikan wifi di depan juri.** Tulis, sunting, cari. Tutup aplikasi, buka lagi | Aplikasi tidak bergantung server untuk penggunaan dasar |
...
> menghadirkan sinkronisasi awan tanpa meminta pengguna percaya bahwa server tidak akan pernah
...
> Arsitekturnya dirancang agar server tidak membutuhkan kemampuan itu sejak awal.
...
Yang akan terlihat: alamat, nomor rekening, nomor HP, email, dan nama orang, dengan skor paparan di
...
| Server produksi bermasalah | Jalankan `pnpm dev` lokal; seluruh alur demo tidak butuh internet kecuali bagian sinkronisasi |
| Database tidak terjangkau | Alur offline tetap penuh: tulis, baca, sunting, cari, ganti tema, Penyaring Identitas |
