...
privat dienkripsi di perangkat pengguna sebelum sinkronisasi, sehingga peladen menyimpan replika
...
dikelompokkan dalam map manila per bulan, dengan lima tema yang bisa diganti.
...
layanan, konfigurasi peladennya, kontrol akses basis datanya, dan kualitas penanganan insidennya.

Cloister menghadirkan kenyamanan sinkronisasi awan tanpa meminta kepercayaan itu. Peladen tidak
...
peladen hanya `ciphertext`, `nonce`, `wrapped_dek`, `dek_nonce`, dan `size_bucket`.
...
Risiko terbesar dari fitur penerbitan bukan pada peladen, melainkan pada penggunanya sendiri: orang
...
berisi `<article>`. Lima tema, dua mode, semuanya lolos kontras WCAG AA yang diperiksa otomatis di
tes.
...
Yang bisa dicoba **tanpa akun**: `/` halaman muka, `/baca` feed publik, `/keamanan` hierarki kunci
dan model ancaman, `/privasi` kebijakan privasi dalam bahasa manusia.
...
```bash
pnpm install
cp .env.example .env          # isi DATABASE_URL dan rahasia peladen
pnpm db:migrate
pnpm dev
```
...
| Styling | Tailwind CSS 4 + CSS custom properties | Lima tema butuh pergantian variabel runtime yang tidak bisa dikerjakan utility class saja |
...
| Basis data | Postgres (Neon di produksi) | Pooling untuk serverless, branching untuk preview deploy |
| ORM | Drizzle | Type-safe, SQL-first, migrasi berbasis berkas, runtime kecil |
| Basis data lokal | Dexie 4 (IndexedDB) | API bersih, transaksi, cocok dengan runes |
...
| 2. Yang dikirim ke peladen | Muatan permintaan sungguhan dari sesi ini, apa adanya |
| 3. Yang tersimpan di basis data | Baris `entries` yang sesungguhnya, nama kolom asli Postgres |
...
plaintext yang pernah dikirim ke peladen**, dihitung dari instrumentasi klien terhadap seluruh
...
| [`tests/unit/kontras.test.ts`](tests/unit/kontras.test.ts) | Lima tema x dua mode lolos kontras WCAG AA |
...
**Yang tidak akan kami klaim.** Penyimpanan terenkripsi melindungi dari paparan di sisi peladen,
tetapi tidak bisa mengubah perangkat atau peramban yang sudah dikompromikan menjadi lingkungan yang
...
**Menulis** — editor markdown dengan pratinjau dan autosave debounce tanpa tombol simpan, satu atau
lebih catatan per tanggal, pemilih suasana hati, tag, lampiran gambar terenkripsi dengan EXIF
dibuang, lokasi dan cuaca opsional, prompt harian.
...
passkey sebagai faktor kedua, kunci ruang dengan PIN lokal, Mode Diperkuat yang membuat peladen
...
**Tampilan** — lima tema x dua mode, responsif 360 sampai 1920 px, `prefers-reduced-motion`
...
```
Perangkat Pengguna                        Peladen
──────────────────                        ───────
Antarmuka SvelteKit                       Rute peladen SvelteKit (Node)
  ├─ Pekerja Kriptografi (libsodium WASM)  ├─ Postgres (Drizzle)
  │    └─ kunci utama hidup di sini,       ├─ Blob storage (lampiran terenkripsi)
  │       tidak pernah keluar              └─ Redis (rate limit, opsional)
  └─ Pekerja Penyaring Identitas
...
2. Rute peladen privat tidak pernah menerima bidang bernama `content`, `title`, `body`, atau apa pun
   yang berisi plaintext catatan.
3. IndexedDB adalah sumber kebenaran bagi pengguna. Peladen adalah replika terenkripsi.
...
kunci autentikasi (dikirim ke peladen, di-hash ulang dengan pepper di sana) dan KEK (tidak pernah
...
- **Seluruh sistem visual.** Tesis papan flanel, palet lima tema, arah cahaya tunggal dari kiri
...
|---|---|
| Perlindungan data dan keamanan informasi | Enkripsi di perangkat, rute privat hanya menerima muatan terenkripsi, pembungkusan kunci berlapis |
| Edukasi mengenai keamanan siber | Halaman Bukti, halaman privasi yang menjelaskan batas kepercayaan peladen, model ancaman yang terbuka |
...
| Pemanfaatan AI secara etis | Pemindaian berjalan di peramban, tidak ada teks yang dikirim ke layanan mana pun, dan bisa dibuktikan dengan tab Network kosong |
