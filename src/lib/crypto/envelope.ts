...
/**
 * Coba buka sebuah catatan dengan kunci utama acak (PRD 14.2).
 *
 ...
 * tag Poly1305 apa adanya, termasuk pesan galat dari libsodium. Kalau fungsi ini
 * suatu hari berhasil, ada yang jauh lebih salah daripada halaman Bukti.
 */
export async function cobaKunciSalah(
	entryId: string,
	e: Pick<EncryptedEntry, 'ciphertext' | 'nonce' | 'wrappedDek' | 'dekNonce'>
): Promise<{ kunciAcak: Uint8Array; berhasil: boolean; galat: string; langkah: string }> {
	...
	try {
		...
		dek.fill(0);
		padded.fill(0);
		return { kunciAcak, berhasil: true, galat: '', langkah: 'ciphertext catatan' };
	} catch (err) {
		return {
			kunciAcak,
			berhasil: false,
			galat: (err as Error).message || String(err),
