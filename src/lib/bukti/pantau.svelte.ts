/**
 * Instrumentasi klien untuk halaman Bukti.
 *
 * Setiap kali klien memanggil rute privat, payload yang benar-benar dikirim
 ...
 * gagal: payload diurai, lalu setiap nilai string diperiksa apakah ia terlihat
...
export interface RekamanKirim {
	...
	metode: string;
	path: string;
	/** Payload apa adanya, sebelum dikirim. */
	payload: string;
	byte: number;
...
class Pantau {
	...
	catat(metode: string, path: string, payload: string) {
		if (!browser) return;
		if (RUTE_PUBLIK.some((r) => path.startsWith(r))) return;

		const byte = new TextEncoder().encode(payload).length;
		const bytePlaintext = hitungPlaintext(payload);

		this.byteTerkirim += byte;
		this.bytePlaintext += bytePlaintext;
		this.rekaman = [
			{ waktu: Date.now(), metode, path, payload, byte, bytePlaintext },
			...this.rekaman
	...
	untukEntri(entryId: string): RekamanKirim | null {
		return this.rekaman.find((r) => r.payload.includes(entryId)) ?? null;
	}
 ...
 */
export function hitungPlaintext(payload: string): number {
	let data: unknown;
	try {
