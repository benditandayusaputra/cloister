...
/**
 * Payload notifikasi tidak boleh berisi isi jurnal. Server memang tidak punya
...
export async function kirimBatch(langganan: Langganan[], n: Notifikasi): Promise<number> {
	...
	const payload = JSON.stringify({ judul: n.judul, isi: n.isi, url: n.url ?? '/app/hari-ini' });
	const mati: string[] = [];
	let terkirim = 0;
	...
	await Promise.all(
		langganan.map(async (s) => {
			try {
				...
				// 404 dan 410 berarti langganan sudah mati di sisi browser.
