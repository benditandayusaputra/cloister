...
function dariFrontmatter(teks: string, namaBerkas: string): LocalEntry | null {
	const m = /^---\n([\s\S]*?)\n---\n?([\s\S]*)$/.exec(teks);
	...
	const tanggal = isoDari(ambil('date')) ?? isoDari(namaBerkas);
...
export async function imporBerkas(file: File): Promise<HasilImpor> {
	...
	throw new Error('Format berkas tidak dikenal. Pakai .zip, .json, atau .md');
}
...
async function imporZip(file: File): Promise<HasilImpor> {
	...
	let masuk = 0;
	let dilewati = 0;
	const berkasMd = zip.file(/\.md$/);
	for (const f of berkasMd) {
