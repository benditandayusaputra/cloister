	...
	<main class="shell" style="display:flex;flex-direction:column;gap:var(--s-6);padding-bottom:var(--s-9)">
		...
		<header style="display:flex;flex-direction:column;gap:10px">
			...
			>
				...
				yang dikirim ke peladen, dan baris apa yang benar-benar tersimpan di basis data. Semuanya
				diambil langsung dari sistem yang sedang berjalan, bukan tangkapan layar.
			</p>
			<p
		...
		<!-- Penghitung byte plaintext (PRD 14.2) -->
		<section
		...
		>
			<div style="display:flex;flex-direction:column;gap:4px">
				<span class="t-data t-data-ink">Byte plaintext yang pernah dikirim ke peladen</span>
				<span
					class="t-hand"
		...
		{#if pilihan}
			<div
				style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:var(--s-5);align-items:start"
			>
				<!-- Panel 1 -->
				...
				>
					<span class="t-data t-data-ink">2 · Yang dikirim ke peladen</span>
					{#if kirimTerakhir}
						...
						<pre
							style="margin:0;max-height:320px;overflow:auto;font-family:var(--f-data);font-size:0.72rem;line-height:1.5;color:var(--ink);background:rgb(27 27 23 / 0.05);padding:10px;border-radius:4px;white-space:pre-wrap;word-break:break-all">{kirimTerakhir.muatan}</pre>
					{:else if amplop}
						<span style="font-family:var(--f-read);font-size:0.82rem;color:var(--ink-soft)">
							...
							muatannya identik dengan yang dikirim mesin sinkronisasi.
						</span>
						<pre
				...
				<section
					...
					style="padding:var(--s-5);display:flex;flex-direction:column;gap:12px"
				>
					<span class="t-data t-data-ink">3 · Yang tersimpan di basis data</span>
					{#if barisDb}
						<span style="font-family:var(--f-read);font-size:0.82rem;color:var(--ink-soft)">
					...
					{:else if galatDb}
						<p class="t-baca" style="font-size:0.86rem;color:var(--ink-soft)">
							{galatDb}. Catatan ini belum pernah sampai ke peladen — itu sendiri sudah jawaban:
			...
			>
				...
				<p class="t-baca" style="font-size:0.9rem;color:var(--ink-soft);max-width:70ch">
					...
					termasuk pesan galat dari libsodium.
				</p>
				<button
					type="button"
					class="tbl"
				...
				{#if percobaan}
					...
					>
						...
						<dt style="color:var(--ink-soft)">galat</dt>
						<dd style="margin:0;color:var(--ink);word-break:break-word">{percobaan.galat}</dd>
					</dl>
				{/if}
			</section>
		{/if}
